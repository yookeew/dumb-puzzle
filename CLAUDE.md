...
Stage 2 scope decision (2026-09-02): first pass is Option A — internal features only. Families 2/3/4 in full, plus a cheap physical baseline (unimpeded taxi percentile per stand-group/runway/type/wake, NO routed distance). Then Stage 3a. OSM is deferred — see "Future step B" below.
FUTURE STEP B — OSM routed taxi distance. Build a per-airport routable graph from an OpenStreetMap extract (taxiways, aprons, runways), match stand IDs to coordinates from a per-airport layout source, shortest-path each stand→runway-threshold, emit data/external/routed_distances.parquet (ODbL: carry data/external/LICENSE + "© OpenStreetMap contributors" + a DATA_SOURCES.md row). Slots into family 1 as one column `routed_distance_m` and into the Stage 3d reference model; costs one retrain, no architecture change. Do this once Option A has a real LightGBM standing and the increment is worth the geometry work.
...
Stage 2 families 1–4 (Option A, internal-only), stage 3a. [DONE 2026-09-02 — holdout RMSE 301s vs 400s baseline, submission lgbm_v1. reports/stage2_3a.md.]
LIRF (Rome) investigated [reports/lirf_investigation.md]: 18% "echo" rows (block==sched -> taxi absorbs departure delay) + genuine huge tail. Per-airport residual heads tried [reports/stage3a_resid.md] -> REGRESSION, abandoned; lgbm_v1 still best.
Training is a bottleneck (CPU fit ~4min, pipeline ~25min). Split done: features local (Polars), model on Colab -> src/features/export_model_inputs.py + src/models/fit.py + notebooks/colab_train.py.
NEXT (on Colab): robust loss on d (Huber/quantile) for the LIRF tail; out-of-fold echo-rate + mean-d features per (airport,operator)/(airport,stand); then OSM routed distance (Future step B), weather ingest, stage 3b.
...
Let us know when you would want us to train in Google Colab than natively
